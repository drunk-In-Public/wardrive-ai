...
class KMLParser {
  ...
  _extractNetworkInfo(placemarkEl) {
    ...
    // Full description text — WiGLE format:
    // "Network ID: XX:XX:XX:XX\nEncryption: WPA2\nSignal: -93.0\n..."
    ...
    const bssid    = field("Network ID");
    const signal   = field("Signal");
    const encField = field("Encryption");
    const type     = field("Type");

    // Security: prefer the explicit Encryption field, fall back to full desc scan
    let security = encField ? this._classifySecurity(encField) : "Unknown";
    if (security === "Unknown") {
      // Try SimpleData / Data elements
      for (const sd of allEls.filter(e => e.localName === "SimpleData" || e.localName === "value")) {
        const s = this._classifySecurity(sd.textContent);
        if (s !== "Unknown") { security = s; break; }
      }
    }
    if (security === "Unknown") security = this._classifySecurity(desc);

    return { ssid, bssid, signal, type, security };
  }
...
function drawKmlTracks(tracks, filename) {
  ...
  tracks.forEach((track) => {
    ...
    if (track.isPointCollection) {
      ...
      const step       = Math.ceil(totalPts / MAX_DOTS);
      const secCounts  = {};
      let drawn = 0;

      for (let i = 0; i < totalPts; i += step) {
        let lat, lon, secColor, ssid, security, bssid, signal;
        ...
        if (hasMetadata) {
          ...
          secColor   = color;
          ssid = "Network"; security = "Unknown"; bssid = ""; signal = "";
        }

        // Use Leaflet's default renderer (SVG) — most compatible across browsers.
        // Do NOT pass a custom renderer instance; let Leaflet manage it.
        const dot = L.circleMarker([lat, lon], {
          radius:      7,
          color:       "#000",        // thin black outline for visibility
          weight:      0.5,
          fillColor:   secColor,
          fillOpacity: 0.85,
          interactive: true,
        ...
        dot.bindPopup(
          `<div style="font:13px/1.6 monospace;min-width:160px">
            <b style="font-size:14px">${ssid}</b><br>
            <span style="color:${secColor};font-weight:700">⬤ ${security}</span>
            ${bssid  ? `<br><span style="color:#888;font-size:11px">${bssid}</span>` : ""}
            ${signal ? `<br>Signal: ${signal} dBm` : ""}
            <br><span style="color:#aaa;font-size:10px">${lat.toFixed(5)}, ${lon.toFixed(5)}</span>
          </div>`,
          { maxWidth: 240 }
        );
      ...
      if (hasMetadata && drawn > 0) addSecurityLegend();
      return;
    }
