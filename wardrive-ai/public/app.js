...
class KMLParser {
  ...
  parse(kmlText) {
    ...
    for (const pm of byLocal("Placemark")) {
      ...
      if (coords.length > 0) {
        const info = this._extractNetworkInfo(pm, pmIdx++);
   ...
   */
  _extractNetworkInfo(placemarkEl, debugIdx) {
    ...
    // Debug: log the raw description of the first 3 placemarks
    if (debugIdx < 3) {
      console.log(`[KMLParser] Placemark[${debugIdx}] SSID="${ssid}" desc="${desc.slice(0, 300)}"`);
    }
