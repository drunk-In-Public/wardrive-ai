...
class KMLParser {
   ...
   * Handles WiGLE KML which stores info in <description> or <ExtendedData>.
   */
  _extractNetworkInfo(placemarkEl) {
    ...
    // Try <ExtendedData><Data name="Encryption"><value>...</value>
    const dataEls = Array.from(placemarkEl.getElementsByTagName("*")).filter(e => e.localName === "Data");
    for (const d of dataEls) {
      const attrName = (d.getAttribute("name") || "").toLowerCase();
      if (attrName === "encryption" || attrName === "security" || attrName === "capabilities") {
        const valEl = Array.from(d.getElementsByTagName("*")).find(e => e.localName === "value");
        if (valEl) {
    ...
    // Fall back to <description> text search
  ...
  _classifySecurity(text) {
    ...
    if (t.includes("OPEN") || t.includes("[ESS]") || t.includes("NONE") || t === "") return "Open";
    return "Unknown";
  }
