...
class KMLParser {
  ...
  parse(kmlText) {
    ...
    // ── 4. Points ─────────────────────────────────────────────
    const points = [];
    for (const pt of byLocal("Point")) {
      const coords = this._parseKmlCoords(pt);
      if (coords.length > 0)
        points.push({ name: this._nearestName(pt, doc), lat: coords[0][0], lon: coords[0][1] });
    }

    // ── 5. Fallback: if no tracks but many points (WiGLE network export) ──
    //    Mark as a point collection so it draws as dots, not connected lines.
    if (tracks.length === 0 && points.length >= 2) {
      console.log(`[KMLParser] No tracks — treating ${points.length} points as scan locations`);
      tracks.push({
        name: "Scan Locations",
...
function drawKmlTracks(tracks, filename) {
  ...
  tracks.forEach((track) => {
    ...
    // ── Point collection (WiGLE network export, etc.) ──────────
    // Draw as individual scan dots — never connect into lines.
    if (track.isPointCollection) {
      const MAX_DOTS = 5000; // cap for performance
      const step = Math.ceil(track.coords.length / MAX_DOTS);
      let drawn = 0;
      for (let i = 0; i < track.coords.length; i += step) {
        const [lat, lon] = track.coords[i];
      ...
      console.log(`[KML] Drew ${drawn} scan dots from ${track.coords.length} points`);
      return;
    }
