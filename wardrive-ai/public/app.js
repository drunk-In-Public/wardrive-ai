...
class KMLParser {
  ...
  parse(kmlText) {
    ...
    // ── 5. Fallback: if no tracks but many points, treat as one track ──
    //    (handles WiGLE export & apps that log individual waypoints)
    if (tracks.length === 0 && points.length >= 2) {
      console.log(`[KMLParser] No tracks found — converting ${points.length} points to a track`);
      tracks.push({
        name: "GPS Points",
