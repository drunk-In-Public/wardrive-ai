...
class KMLParser {
   ...
   * Handles: LineString, gx:Track, MultiGeometry.
   * Uses getElementsByTagName(NS) for reliable namespace handling
   * across all browsers (querySelectorAll is unreliable with gx: prefix).
   */
  parse(kmlText) {
    const parser = new DOMParser();
    // Parse as XML first; fall back to HTML parser if it fails
    ...
    // ── 1. Standard KML LineStrings ──────────────────────────
    const lineStrings = Array.from(doc.getElementsByTagName("LineString"));
    for (const ls of lineStrings) {
      const coords = this._parseKmlCoords(ls);
      if (coords.length >= 2) {
        tracks.push({ name: this._nearestName(ls) || "Track", coords });
      }
    }

    // ── 2. gx:Track (WiGLE, Google Earth, GPS Logger) ────────
    // Try both namespace-aware and prefix-based lookup for compatibility
    const GX_NS = "http://www.google.com/kml/ext/2.2";
    const gxTrackEls = [
      ...Array.from(doc.getElementsByTagNameNS(GX_NS, "Track")),
      ...Array.from(doc.getElementsByTagName("gx:Track")),
    ];
    const seenGx = new WeakSet();
    for (const gt of gxTrackEls) {
      if (seenGx.has(gt)) continue;
      seenGx.add(gt);
      const coords = this._parseGxTrack(gt, GX_NS);
      if (coords.length >= 2) {
        tracks.push({ name: this._nearestName(gt) || "Track", coords });
      }
    }

    // ── 3. gx:MultiTrack ─────────────────────────────────────
    const gxMultiTrack = [
      ...Array.from(doc.getElementsByTagNameNS(GX_NS, "MultiTrack")),
      ...Array.from(doc.getElementsByTagName("gx:MultiTrack")),
    ];
    const seenMt = new WeakSet();
    for (const mt of gxMultiTrack) {
      if (seenMt.has(mt)) continue;
      seenMt.add(mt);
      const innerTracks = [
        ...Array.from(mt.getElementsByTagNameNS(GX_NS, "Track")),
        ...Array.from(mt.getElementsByTagName("gx:Track")),
      ];
      const seenInner = new WeakSet();
      for (const gt of innerTracks) {
        if (seenInner.has(gt)) continue;
        seenInner.add(gt);
        const coords = this._parseGxTrack(gt, GX_NS);
        if (coords.length >= 2) {
          tracks.push({ name: this._nearestName(mt) || "Track", coords });
        }
      }
    }

    // ── 4. Points ─────────────────────────────────────────────
    for (const pt of Array.from(doc.getElementsByTagName("Point"))) {
      const coords = this._parseKmlCoords(pt);
      if (coords.length > 0) {
        points.push({ name: this._nearestName(pt) || "Point", lat: coords[0][0], lon: coords[0][1] });
      }
    }

    console.log(`[KMLParser] Found ${tracks.length} tracks, ${points.length} points`);
    tracks.forEach((t, i) =>
      console.log(`  Track ${i+1}: "${t.name}" — ${t.coords.length} pts`)
    );
  ...
  /** Parse a gx:Track element's gx:coord children → [[lat,lon], ...] */
  _parseGxTrack(gt, GX_NS) {
    const coords = [];
    const coordEls = [
      ...Array.from(gt.getElementsByTagNameNS(GX_NS, "coord")),
      ...Array.from(gt.getElementsByTagName("gx:coord")),
    ];
    const seen = new WeakSet();
    for (const c of coordEls) {
      if (seen.has(c)) continue;
      seen.add(c);
      // gx:coord format: "lon lat [ele]" (space-separated)
  ...
  /** Walk up the DOM to find the nearest <name> text content */
  _nearestName(el) {
    let node = el;
    while (node && node.nodeName !== "Document") {
      const nameEl = node.getElementsByTagName("name")[0];
      if (nameEl && nameEl.parentNode === node) return nameEl.textContent.trim();
